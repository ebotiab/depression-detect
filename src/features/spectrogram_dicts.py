...
def build_class_dictionaries(dir_name):
    ...
    depressed_dict : dictionary
        a dictionary of depressed individuals with keys of participant id
        and values of with the matrix spectrogram representation
    normal_dict : dictionary
        a dictionary of non-depressed individuals with keys of participant id
        and values of with the matrix spectrogram representation
    """
    depressed_dict = dict()
    normal_dict = dict()
    for subdir, dirs, files in os.walk(dir_name):
        for file in files:
            if file.endswith('no_silence.wav'):
                participant_id = int(file.split('_')[0][1:])
                if in_dev_split(participant_id):
                    ...
                    if depressed:
                        depressed_dict[participant_id] = mat
                    else:
                        normal_dict[participant_id] = mat
    return depressed_dict, normal_dict


def in_dev_split(participant_id):
    """
    Returns True if the participant is in the AVEC development split
    (aka participant's we have depression labels for)
    """
    return participant_id in set(df_dev['Participant_ID'].values)
...
def get_depression_label(participant_id):
    ...
    return df_dev.loc[df_dev['Participant_ID'] ==
                      participant_id]['PHQ_Binary'].item()


if __name__ == '__main__':
    dir_path = '../../data/interim'
    depressed_dictionary, normal_dictionary = build_class_dictionaries(dir_path)
